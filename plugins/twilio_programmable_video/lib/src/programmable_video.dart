...
class TwilioProgrammableVideo {
  ...
  static Future<bool> requestPermissionForCameraAndMicrophone({bool camera = true}) async {
    final permissions = camera ? [Permission.camera, Permission.microphone] : [Permission.microphone];
    await permissions.request();
    
    final micPermission = await Permission.microphone.status;
    final camPermission = camera ? await Permission.camera.status : PermissionStatus.granted;
    ...
    if (micPermission == PermissionStatus.denied || (camera && camPermission == PermissionStatus.denied)) {
      return requestPermissionForCameraAndMicrophone(camera: camera);
    }
  ...
  static Future<Room> connect(ConnectOptions connectOptions) async {
    final needsCamera = connectOptions.videoTracks != null && connectOptions.videoTracks!.isNotEmpty;
    if (await requestPermissionForCameraAndMicrophone(camera: needsCamera)) {
      try {
